...
The response to the request for retrieving the company’s API keys is a JSON object containing two key pieces of information:
...
| `components_push.body_text_1` | Static body text of the message. In this case, it shows the name of the course or service (e.g., "Técnico en Cuidados Auxiliares de Enfermería").  |
...
```json
{
  "components_push": {
    "body_text_0": "{{name}}",
    "body_text_1": "Técnico en Cuidados Auxiliares de Enfermería"
  },
  "contact": {
    "number": "34666033135",
    "name": "Jierson"
...
> ⚠️ **Attention: Remember that `name_template` must be in lowercase, and spaces are not allowed
...
⚠️ In case of an error during template creation, the response will provide a message detailing the issue. For example, if the name_template format is incorrect, you may receive an error message similar to the one below:
...
### 📥 Example of Incoming WhatsApp Webhook Simulation
